...
- Docs in `docs/`: `components/` (per-module reference for current functioning — start at `components/README.md`), `legacy_language_model_analysis.md` (the language model + a prioritized list of known model weaknesses), `legacy_hdf5_data.md` (the now-dropped HDF5 files, kept for historical context), and `superpowers/plans/2026-05-14-tech-stack-modernization.md` (the modernization plan and execution log).
